...
- **説明**: 軽量で高速に動作する
- **詳細**:
  - Go言語によるコンパイル済みバイナリで軽量・高速
  - Svelteによるコンパイル時最適化で軽量なフロントエンド
  - 最小限の依存関係
  - Dockerイメージのサイズ最小化（マルチステージビルド）
...
- **要件ID**: TECH-001
- **技術スタック**:
  - **Svelte** - 軽量でパフォーマンスが良いフロントエンドフレームワーク
  - SvelteKit（必要に応じて）または純粋なSvelte
  - モダンなUIフレームワーク（Tailwind CSSなど）
  - TypeScript（推奨）

### 4.2 バックエンド
- **要件ID**: TECH-002
- **技術スタック**:
  - **Go（Golang）** - 軽量で高速なバックエンド実装
  - HTTPサーバー: net/http または 軽量フレームワーク（Gin、Echoなど）
  - CORS制約の回避と脆弱性検査の実行
  - 非同期処理での検査タスクの管理

### 4.3 Docker構成
- **要件ID**: TECH-003
- **説明**: Dockerを使用した開発・本番環境の統一
- **詳細**:
  - **マルチステージビルド**を採用
  - **フロントエンドコンテナ**: Svelteアプリケーションのビルドと提供
  - **バックエンドコンテナ**: Goアプリケーションの実行
  - **docker-compose.yml**によるサービス統合
  - 開発環境と本番環境の両方に対応
...
- **説明**: システム構成
- **詳細**:
  - **フロントエンド（Svelte）**: ユーザーインターフェース、URL入力、結果表示
  - **バックエンド（Go）**: RESTful API、脆弱性検査ロジック、CORSプロキシ機能
  - **通信**: HTTP/HTTPS（JSON形式のAPI）
  - **コンテナ構成**: Docker Composeでフロントエンドとバックエンドを統合
...
- **詳細**: 
  - クロスオリジンリクエストがブラウザで制限される場合がある
  - Goバックエンドがプロキシとして機能し、CORS制約を回避
  - 一部のサイトではCORS以外のセキュリティ対策によりアクセスできない場合がある
